...
class DiscordBot(object):
    ...
    def send_message(self, text, embed: bool = True):
        #Sends a message to the chatroom
        if embed:
            message = "```{0}```".format(text)
        else:
...
def get_activity(league, week=None):
    ...
    #current = datetime.datetime(2021, 9, 23, 10, 30, 0)
    ...
    for a in activity:
        for action in a.actions:
            ...
            print('\t', ''.join(str(current - datetime.timedelta(minutes=1)).split(":")[0:2]) , "==", ''.join(str(action_datetime).split(":")[0:2]))
...
def utc_to_local(utc_dt):
    return utc_dt.replace(tzinfo=datetime.timezone.utc).a
